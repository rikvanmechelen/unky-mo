...
- `internal/tui/sidebar/` keeps its own smaller `TmuxClient` / `ClaudeReader` interfaces in `sidebar/deps.go` since the sidebar runs in a separate process and reaches for a different subset (`BreakPane`, `JoinPane*`, `ActiveShellsForSession`, `FormatShellCommand`). These are generated separately into `sidebar/mocks/`.
