...
- `cmd/mo/` — CLI entry point (Cobra), all subcommands. Each `RunE` builds an `ops.Context` and calls an `ops.*` function — thin wrappers by design.
- `internal/ops/` — **Domain operations shared by CLI and TUI.** `Context` + interfaces (`TmuxClient`, `ClaudeReader`) + functions: `LaunchSession`, `CreateWorktreeAndLaunch`, `OpenBranchInMain`, `ParkAndLaunch`, `LaunchSibling`, `ImportExternalSession`, `CleanupWorktree`, `ResumeInDir`, `SessionToWindowMap`, `PrimaryWindowForTarget`, `SignalAndWaitExit`. No bubbletea types here — plain functions, testable against gomock fakes.
...
- `internal/tui/sidebar/` — Compact sidebar TUI for tmux panes (model, styles, run). Has its own `TmuxClient`/`ClaudeReader` interfaces since it runs in a separate process and needs a different method subset.
...
- `internal/github/` — GitHub PR fetching via `gh` CLI, behind a `Client` constructed with a `moexec.Commander`.
...
The sidebar manages a collapsible terminal drawer below the Claude pane (pane `.0`). Only one terminal tab is visible at a time — inactive tabs are parked in a dedicated mo-terms tmux session (`tmux.MoTermsSession` is the base name; each sidebar scopes it per-window via `Model.termSession()` → `mo-terms-<windowID>` so tabs from `moma-chatbot` don't show up in `moma-apps-rails`'s backtick popup). Panes are moved in and out of the project window via cross-session `break-pane`/`join-pane`. The sidebar tracks terminal pane IDs (`%N` format, stable across moves) and prunes dead panes on each 1s refresh tick.

Backtick (`` ` ``) opens a floating popup that attaches a nested tmux client to `mo-terms` — so the popup shows the same shells as the drawer. `mo-terms` sets its `key-table` session option to `popup-keys`, where the sidebar binds `` ` `` → `detach-client`. That makes backtick close the popup without killing the shell. Tab/Shift+Tab intentionally pass through to the shell — tab completion matters more than in-popup tab cycling; use the sidebar's `t`/`tab` on the project window instead. The outer `mo` client stays on the default `root` table, so backticks remain typeable in Claude panes. If the drawer is open when `` ` `` is pressed, the active tab is first parked in `mo-terms` so it shows up in the popup; the drawer is not auto-restored on popup close (the user can hit `t` to reopen).
...
- **`ProjectState`** — one entry **per live Claude session**, plus one placeholder per zero-session known project (so the sidebar still lists empty projects with a dim dot). Concurrent siblings / renamed primaries produce separate entries, distinguished by `WindowID` (stable) / `WindowName` + `SessionID`. Fields: `name`, `path`, `window_name`, `window_id` (stable tmux id, e.g. `@5` — survives renames; empty on placeholder rows and mid-launch sessions whose window isn't yet resolved), `status` (`"none"` | `"active"` | `"idle"` | `"permission"` | `"external"`), `parent` (for worktree/sibling rows), `section` (`"projects"` | `"external"`), `branch` + `dirty` (git-backed strays only), `session_id`, `index` (0 = primary, 2+ = sibling ordinal, -1 = custom-title window). Sidebars prefer `window_id` over `window_name` when matching their own row so the "current window" bold/underline highlight survives `/rename` and ordinal shuffling; they fall back to name equality when either side has no id.
...
- Unit tests live alongside code. Covered packages include `claude`, `config`, `notify`, `state`, `project` (real-git tempdirs), `sync`, `tickets`, `tickets/jira` (httptest-mocked `/rest/api/3/search/jql`), `tmux`, `usage`, plus pure helpers in `tui` and `tui/sidebar`, a `cmd/mo/jira_test.go` wizard suite, and a shared `internal/exec.Commander` seam (mocked via gomock) used by `github`.
- **Make targets**: `make test`, `make test-race`, `make test-expectfail`, `make test-integration`, `make mocks` (regenerate mocks after changing an interface), `make mocks-check` (CI idempotency check).
- **Mocking**: interfaces carry `//go:generate mockgen ...` directives. Generated mocks live under `<pkg>/mocks/` and are committed (contributors don't need `mockgen` installed to run tests). CI runs `make mocks` and fails if the diff isn't empty, so regenerate + commit whenever you change a mocked interface.
- **Integration tests** (`//go:build integration`) spin up an isolated tmux server via `tmux -L <unique-socket>` and a fake `claude` binary (`internal/tmux/testdata/fake-claude.sh`) to exercise real session detection, PPID-chain attribution, and JSONL idle reads end-to-end. Tests live in `internal/integration/` and `internal/tmux/client_integration_test.go`. They set `SHELL=/bin/sh` on the tmux server to bypass the user's interactive-shell first-run wizards (zsh-newuser, fish config).
...
- The CLI's subcommands (`cmd/mo/*.go`) are the same pattern: each `RunE` builds a `*ops.Context` via `ops.NewContext(tmuxClient)` and calls an `ops.*` function. `mo start`, `mo park`, `mo cleanup`, `mo import`, `mo concurrent` all go through this path — adding a new subcommand for an existing op takes ~30 lines of cobra boilerplate.
