...
- The TUI (`internal/tui/`) and sidebar (`internal/tui/sidebar/`) expose two dependency seams — `TmuxClient` and `ClaudeReader` — defined in each package's `deps.go` with `//go:generate mockgen` directives. Production code uses `NewModel(...)` (wraps `*tmux.Client` and the `claude` package via adapters); tests use `NewModelWithDeps(...)` or construct `Model{tmux: mockTmux, claude: mockClaude}` directly. See `internal/tui/handlers_test.go` for examples driving `primaryWindowForTarget`, `sessionToWindowMap`, `resolveSessionWindows`, and `applyNotifOverrides` with mocked deps.
- When adding a new tmux method call or `claude.*` package call inside the TUI, add the method to the interface in `deps.go`, update the adapter, run `make mocks`, and commit the regenerated mock. Never reach past the interface to the underlying package from inside a `Model` method — the interface is what tests can observe.
