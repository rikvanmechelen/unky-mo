...
- No automated UI tests for the main TUI (`internal/tui/`) — UI correctness is validated manually. When adding non-trivial logic, extract it out of the bubbletea `Update` into a pure helper so it can be covered without a terminal harness. `internal/tui/helpers_test.go` + `internal/tui/sidebar/helpers_test.go` cover the already-extracted pure functions (status ranking, window-name parsing, file-tree rendering, etc.).
